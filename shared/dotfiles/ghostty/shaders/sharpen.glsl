float SHARPEN_STRENGTH = 1.00;
float RADIUS_PX = 1.00;
float EDGE_THRESHOLD = 0.010;
float CLAMP_MAX = 0.080;
float SATURATION_BOOST = 1.15;
float CONTRAST_BOOST = 1.05;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Boost saturation
  float grey = luma(outColor);
  outColor = mix(vec3(grey), outColor, SATURATION_BOOST);

  // Subtle contrast boost
  outColor = (outColor - 0.5) * CONTRAST_BOOST + 0.5;
